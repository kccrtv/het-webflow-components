...
exports.atomWithSearchParams = atomWithSearchParams;
const vanilla_1 = require("jotai/vanilla");
const atomWithLocation_js_1 = require("./atomWithLocation.js");
...
// Implementation
function atomWithSearchParams(key, defaultValue, options) {
    var _a;
    // Create an atom for managing location state, including search parameters.
    const locationAtom = (0, atomWithLocation_js_1.atomWithLocation)({
        ...options,
    ...
    const resolveValue = (value) => {
        ...
        if (typeof defaultValue === 'number') {
            if (value === '') {
                (0, logger_js_1.warning)(`Empty string provided for key "${key}". Falling back to default value.`);
                return defaultValue;
            }
            ...
            (0, logger_js_1.warning)(`Expected a number for key "${key}", got "${value}".`);
            return defaultValue;
        }
        ...
        if (typeof defaultValue === 'boolean') {
            ...
            if (value === 'false')
                return false;
            (0, logger_js_1.warning)(`Expected a boolean for key "${key}", got "${value}".`);
            return defaultValue;
        }
        ...
        (0, logger_js_1.warning)(`Unsupported defaultValue type for key "${key}".`);
        return defaultValue;
    };
    /**
    ...
    const parseValue = (value) => {
        ...
        (0, logger_js_1.warning)(`Unsupported value type for key "${key}":`, typeof value);
        throw new Error(`Unsupported value type for key "${key}".`);
    };
    return (0, vanilla_1.atom)(
